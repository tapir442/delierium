            ...
            for s in e.operands():
                ...
                if is_derivative(s):
                    ...
                    for item in s.operands():
                        (d if (is_derivative(item) or self.ctxfunc(item))
                         else coeff).append(item)
    ...
    def normalize(self):
        if self._p:
            if bool(self._p[0]._coeff == Integer(1)):
                ...
                self._p = [
                    _ for _ in self._p if _._coeff and not bool(_._coeff == 0)]
                c = self._p[0]._coeff
                self._p = [DTerm((_._coeff / c) * _._d, self._context)
                           for _ in self._p]

    def __nonzero__(self):
        return bool(len(self._p))
...
def Autoreduce(S, context):
    ...
    while True:
        ...
        for _r in r:
            ...
            if rnew._p != []:
                # why does __nonzero__ not work
                newdps.append(rnew)
        dps = Reorder(p + [_ for _ in newdps if _ and _ not in p],
...
def complete(l, ctx):
    ...
    for m, monomial in zip(leading_derivatives, l):
        ...
        l.extend(m0)
        l = reversed(
            sorted(map(lambda _: Differential_Vector(_, ctx), list(set(l)))))
        l = [_._e for _ in l]
