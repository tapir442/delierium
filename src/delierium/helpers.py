...
def is_numeric(e):
    return isinstance(
        e, (Integer, Rational, int, float, complex, Zero, One, NegativeOne, Half)
    ...
    if use_func_diff:
        for v in vars:
            if "NewSymbolicFunction" in v.__class__.__name__:
                ...
                xx = SR.var("xx")
                gg = f.subs({context._dependent[0](context._independent[1]): xx})
                gg = diff(gg, v)
                f = gg.subs({xx: context._dependent[0](context._independent[1])})
    else:
        f = f.diff(*vars)
    return f
...
def ltf(expr, dep, indep, infinitesimals=None, printer=True):  # noqa: C901
    ...
    output = finish_substitution(expr)
    used_symbols = {}
    ...
    if len(indep) == 1:
        ...
        dreps2 = {
            deriv: (
                Symbol(deriv.output.subs(coefficient_functions) + ' '.join("'" * deriv.args[-1][1]))
            )
            for deriv in output.atoms(Derivative)
            if deriv.args[0] in dep
        }
    else:
...
def is_jupyter_lab() -> bool:
    """Check if the code is running in a Jupyter Lab environment."""
    try:
...
def lie_derivative_printer(  # noqa: C901
    expressions: list[Expr],
    dependent_vars: list[sp.Symbol],
    independent_vars: list[sp.Symbol],
    infinitesimals: list[sp.Function],
    *args,
    **kwargs,
) -> None:
    """
    Print SymPy expressions in Lie standard form.
    """

    class LiePrinter(LatexPrinter):
        ...
        def _print_Derivative(self, expr):
            ...
            # Get all differentiation variables
            variables = expr.args[1:]

            # For excluded functions with single variable, use prime notation
            if func_name in [_.func.__name__ for _ in self.infinitesimals]:
                subscript = ""
                for v in variables:
                    s = v[0]
            ...
            if len(set(variables)) == 1:
                order = variables[0][1]
                return f"{func_name}{"'" * order}"

            # For Lie notation: concatenate all variables
            subscript = "".join(str(v[0]) * v[1] for v in variables)
            return f"{func_name}_{subscript}"

    class LatexLiePrinter(LatexPrinter):
        def __init__(self, infinitesimals):
            self.infinitesimals = infinitesimals
            super().__init__()
    ...
    for expr in expressions:
        if is_jupyter_lab():
            ...
            latex_printer = LatexLiePrinter(infinitesimals)
            latex_expr = latex_printer.doprint(expr)
            display(Math(latex_expr))
        else:
            printer = LiePrinter(infinitesimals)
...
if __name__ == "__main__":
    ...
    f = sp.Function('f')(x)  # Will be excluded

    # Test cases
    expr1 = sp.Derivative(u, x)  # Should print: u_x
    expr2 = sp.Derivative(u, x, x)  # Should print: u_xx
    expr3 = sp.Derivative(u, x, y)  # Should print: u_xy
    expr4 = sp.Derivative(u, x, x, y)  # Should print: u_xxy
    expr5 = sp.Derivative(f, x, 2)  # Should print: f''
    expr6 = sp.Derivative(u, x, y, x)  # Should print: u_xyx
    expr7 = sp.Derivative(u, x, y, x) * f * sp.Derivative(f, x, 3) + sp.Derivative(u, x, y) * u**2

    lie_derivative_printer(
        expressions=[expr1, expr2, expr3, expr4, expr5, expr6, expr7],
        dependent_vars=[u, v, f],
